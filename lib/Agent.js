...
module.exports = class Agent {
	...
	async initializeHTTPMiddleware() {
		...
		if (!this.options.secret)
			console.warn('Warning:', 'session is not working in safe because secret key is not set.');

		const httpAgent = this.getContext().get('HTTP').getAgent(this.options.httpAgent);

		// Setup middleware to parse authorization header for session
		httpAgent.addMiddleware(async (ctx, next) => {
			...
			// Making sure it is JWT in authorization header
			if (!ctx.headers.authorization.startsWith('JWT ', 0)) {
				await next();
				return;
			}
			...
			try {
				...
				let payload = this.decodeJwtToken(token);
